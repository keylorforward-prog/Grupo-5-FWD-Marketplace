  ...
  if (sent) {
    return (
      <div className="p-6 w-80 text-center">
        ...
        <p className="text-xs text-gray-500 mt-1">Se notificará a {candidateName}</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 w-80">
      <p className="text-xs font-semibold text-brand-600 uppercase tracking-wide mb-3">
        Agendar entrevista — {candidateName}
      </p>
          ...
          return (
            <button
              ...
              className={`w-8 h-8 rounded-lg text-xs font-medium transition-all
                ...
                ${isSelected ? 'bg-brand-600 text-white hover:bg-brand-700 shadow-md' : ''}
                ${isToday && !isSelected ? 'ring-2 ring-brand-300 text-brand-700 font-bold' : ''}
              `}
            >
              {day}
            </button>
