...
exports.getById = async (req, res) => {
  try {
    const data = await Oferta.findByPk(req.params.id);
...
exports.create = async (req, res) => {
  try {
    ...
    const data = await Oferta.create(safeData);
...
exports.update = async (req, res) => {
  try {
    // Prevención de Mass Assignment
    const { id, estado, rol, contrasena, ...safeData } = req.body;
    const [updated] = await Oferta.update(safeData, { where: { id_oferta: req.params.id } });
...
exports.delete = async (req, res) => {
  try {
    const deleted = await Oferta.destroy({ where: { id_oferta: req.params.id } });
