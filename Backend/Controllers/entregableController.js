const { Entregable } = require('../Models');

exports.getAll = async (req, res) => {
  try {
    const data = await Entregable.findAll();
...
exports.getById = async (req, res) => {
  try {
    const data = await Entregable.findByPk(req.params.id);
...
exports.create = async (req, res) => {
  try {
    ...
    const data = await Entregable.create(safeData);
...
exports.update = async (req, res) => {
  try {
    // Prevención de Mass Assignment
    const { id, estado, rol, contrasena, ...safeData } = req.body;
    const [updated] = await Entregable.update(safeData, { where: { id_entregable: req.params.id } });
...
exports.delete = async (req, res) => {
  try {
    const deleted = await Entregable.destroy({ where: { id_entregable: req.params.id } });
