...
exports.getAll = async (req, res) => {
  try {
    const data = await Conversacion.findAll();
...
exports.getById = async (req, res) => {
  try {
    const data = await Conversacion.findByPk(req.params.id);
...
exports.create = async (req, res) => {
  try {
    ...
    const data = await Conversacion.create(safeData);
...
exports.update = async (req, res) => {
  try {
    // Prevención de Mass Assignment
    const { id, estado, rol, contrasena, ...safeData } = req.body;
    const [updated] = await Conversacion.update(safeData, { where: { id_conversacion: req.params.id } });
...
exports.delete = async (req, res) => {
  try {
    const deleted = await Conversacion.destroy({ where: { id_conversacion: req.params.id } });
