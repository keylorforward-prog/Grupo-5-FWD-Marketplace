...
exports.create = async (req, res) => {
  try {
    ...
    const data = await Evaluacion.create(safeData);
...
exports.update = async (req, res) => {
  try {
    // Prevención de Mass Assignment
    const { id, estado, rol, contrasena, ...safeData } = req.body;
    const [updated] = await Evaluacion.update(safeData, { where: { id_evaluacion: req.params.id } });
...
exports.delete = async (req, res) => {
  try {
    const deleted = await Evaluacion.destroy({ where: { id_evaluacion: req.params.id } });
