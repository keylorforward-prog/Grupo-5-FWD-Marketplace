...
exports.getById = async (req, res) => {
  try {
    const data = await Pago.findByPk(req.params.id);
...
exports.create = async (req, res) => {
  try {
    ...
    const data = await Pago.create(safeData);
...
exports.update = async (req, res) => {
  try {
    // Prevención de Mass Assignment
    const { id, estado, rol, contrasena, ...safeData } = req.body;
    const [updated] = await Pago.update(safeData, { where: { id_pago: req.params.id } });
...
exports.delete = async (req, res) => {
  try {
    const deleted = await Pago.destroy({ where: { id_pago: req.params.id } });
